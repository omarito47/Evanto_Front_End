...
export class ReclamationDetailsComponent implements OnInit{
  ...
  editComment(comment: Commentaire) {
    ...
    this.editCommentForm.setValue({ description: comment.description });
  }
  ...
  updateComment() {
    if (this.editingComment) {
      const updatedComment = { ...this.editingComment, description: this.editCommentForm.get('description')?.value };
      this.cs.updateComment(updatedComment._id, this.editingComment).subscribe({
        next: (updated) => {
          console.log(updated);
          this.getAllList();  // Refresh the comments list
          this.cancelEdit();
        },
        error: (err) => alert(err.message)
      });
